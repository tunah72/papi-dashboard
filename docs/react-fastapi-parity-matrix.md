...
- **Sidebar và responsive:** kiểm tại 1440, 1280, 1024, 900, 768 và 390 px. Sáu nhãn target theo thứ tự
  là **Tổng quan; Diễn biến theo thời gian; Vùng & tỉnh; Mối quan hệ lĩnh vực; Thay đổi & phân nhóm;
  Trợ lý AI**; active state đúng, URL trực tiếp vẫn có sidebar. Keyboard đi được qua điều hướng/filter/
...
| `ai_assistant.py` → `/ai-assistant` (**Trợ lý AI**) | Technique/tự nhập/prompt/context; code, explanation, trạng thái, stdout/result/figure và lifecycle log | Bốn bước hiển thị: (1) yêu cầu sinh, (2) code + giải thích, (3) diff + hash-reset khi code đổi, (4) phê duyệt rõ ràng rồi mới có kết quả local/full log. Không hiển thị internal reasoning | API AI nhận request/schema/context; API Thực thi chỉ nhận bản code đã phê duyệt; API Logs lưu `generated_pending_approval` và `executed_after_approval`, code chạy local process con, full result log hoặc lỗi |

## Bằng chứng trước cutover

Mỗi route cần fixture/filter, response reference/API, bảng sai khác numeric, assertion semantic view-model
