...
for (const width of [1440, 1024, 390]) {
  test(`floating assistant không dịch grid và không tràn tại ${width}px`, async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Thu nhỏ' })).toHaveCount(0)
    await page.getByRole('button', { name: 'Phóng to Trợ lý AI' }).click()
    await expect(page.getByRole('dialog', { name: 'Trợ lý AI' })).toHaveClass(/is-maximized/)
    expect(await page.locator('body').evaluate((body) => body.scrollWidth <= innerWidth)).toBe(true)
    const maximizedGrid = await grid.boundingBox()
    expect(maximizedGrid?.x).toBe(before?.x)
    expect(maximizedGrid?.y).toBe(before?.y)
    expect(maximizedGrid?.width).toBe(before?.width)
    await page.keyboard.press('Escape')
    await expect(page.getByRole('dialog', { name: 'Trợ lý AI' })).not.toHaveClass(/is-maximized/)
    await page.getByRole('button', { name: 'Đóng Trợ lý AI' }).click()
    await expect(page.getByRole('dialog', { name: 'Trợ lý AI' })).toBeHidden()
    await expect(page.getByRole('button', { name: 'Mở Trợ lý AI' })).toBeFocused()
  })
}
