...
for (const [width, height] of [[1440,900],[1280,900],[1024,900],[900,900],[768,900],[390,844]] as const) {
  test(`H1-H2 giữ context tại ${width}px`, async ({ page }) => {
    ...
    await expect.poll(async () => page.locator('.js-plotly-plot').evaluateAll((plots) => plots.flatMap((plot) => ((plot as unknown as { _fullData?: Array<{ type: string }> })._fullData ?? []).map((trace) => trace.type)))).toEqual(expect.arrayContaining(['bar','heatmap']))
    ...
    await expect.poll(async () => page.locator('.js-plotly-plot').evaluateAll((plots) => plots.flatMap((plot) => ((plot as unknown as { _fullData?: Array<{ type: string }> })._fullData ?? []).map((trace) => trace.type)))).toEqual(expect.arrayContaining(['box','scatter']))
...
test('H2 xử lý snapshot 2018 có 61 tỉnh và filter URL cũ mà không tạo 422', async ({ page }) => {
  ...
  await expect(page.getByRole('heading',{name:/Khác biệt giữa vùng và tỉnh/})).toBeVisible()
  await expect(page.getByText('61 tỉnh')).toBeVisible()
