...
export function TimeTrend() {
  ...
  return <article className="overview analysis-page time-redesign">
    ...
    <section className="time-chart-grid" aria-label="Bốn biểu đồ diễn biến">
      <ChartCard className="time-chart-card" eyebrow="01 · MỨC ĐIỂM" title="Điểm trung bình các tỉnh tăng, giảm hay ổn định?" insight={<ChartInsight>{d.insights.total}</ChartInsight>} focus={{ ...focus('time-series'), content: <CartesianChart title="Mức điểm theo thời gian" summary="Điểm trung bình theo năm của toàn quốc và sáu vùng." data={lineData} height={560} onHover={(event) => setHoveredRegion(regions[event.points?.[0]?.curveNumber ?? -1] ?? null)} onUnhover={() => setHoveredRegion(null)} layout={{ hovermode: 'x unified', xaxis: { title: { text: 'Năm' }, range: [years[0], years.at(-1)], showspikes: true, spikemode: 'across', spikesnap: 'cursor' }, yaxis: { title: { text: d.totalSeries.unit } }, legend: { orientation: 'h', y: 1.12 }, margin: { l: 64, r: 52, t: 38, b: 50 } }}><TrendTable data={d} /></CartesianChart> }} footer={<ChartMeta source={d.totalSeries.source} unit={d.totalSeries.unit} n={`${trend.data.meta.n} tỉnh–năm`} caveats={d.totalSeries.caveats} />}>
        <CartesianChart title="Mức điểm theo thời gian" summary="Điểm trung bình theo năm của toàn quốc và sáu vùng." data={lineData} height={310} onHover={(event) => setHoveredRegion(regions[event.points?.[0]?.curveNumber ?? -1] ?? null)} onUnhover={() => setHoveredRegion(null)} layout={{ hovermode: 'x unified', xaxis: { range: [years[0], years.at(-1)], showspikes: true, spikemode: 'across', spikesnap: 'cursor' }, yaxis: { title: { text: d.totalSeries.unit } }, legend: { orientation: 'h', y: -0.22 }, margin: { l: 64, r: 44, t: 20, b: 86 } }}><TrendTable data={d} /></CartesianChart>
      </ChartCard>
